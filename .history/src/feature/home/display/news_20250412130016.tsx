...
import { useFormattedBlogs } from '@/feature/news/hooks/useFormattedNewsTitle';


export default function NewsSection(blogsData: any[]) {
  const formattedBlogs = useFormattedBlogs(blogsData);
  // idが1, 2, 3のデータだけ取得
  const newsItems = formattedBlogs.filter((blog) => ['1', '2', '3'].includes(blog.id));
